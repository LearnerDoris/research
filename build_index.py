...
<head>
    ...
    <style>
        :root {{ --bg-color: #0d1117; --grid-line: rgba(48, 54, 61, 0.5); --card-bg: rgba(22, 27, 34, 0.8); --card-border: #30363d; --text-main: #c9d1d9; --text-muted: #8b949e; --accent-glow: #58a6ff; --accent-cyan: #39d353; --hover-bg: #1f242c; }}
        body {{ font-family: 'SF Pro Display', -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; background-color: var(--bg-color); background-image: linear-gradient(var(--grid-line) 1px, transparent 1px), linear-gradient(90deg, var(--grid-line) 1px, transparent 1px); background-size: 40px 40px; color: var(--text-main); max-width: 1100px; margin: 0 auto; padding: 50px 20px; line-height: 1.6; }}
        ...
        document.addEventListener('DOMContentLoaded', () => {{
            ...
            function updateCards() {{
                ...
                cards.forEach(card => {{
                    ...
                    if (matchesFilter && matchesSearch) {{ card.classList.remove('hidden'); visibleCount++; }}
                    else {{ card.classList.add('hidden'); }}
                }});
                noResults.style.display = visibleCount === 0 ? 'block' : 'none';
            }}
            searchInput.addEventListener('input', (e) => {{ searchQuery = e.target.value.toLowerCase(); updateCards(); }});
...
def extract_meta_from_html(filepath):
    ...
    try:
        with open(filepath, 'r', encoding='utf-8', errors='ignore') as f:
            ...
            if title_match and title_match.group(1).strip(): title = title_match.group(1).strip()
...
def generate_site():
    ...
    for root, dirs, files in os.walk('.'):
        if '.git' in root or '.github' in root or root == '.': continue
        ...
        for file in files:
            if file.endswith('.html') and file != 'index.html':
                ...
                total_count += 1
                cards_html += f'\\n        <a href="{{safe_url}}" class="card" data-category="{{folder_name}}">\\n            <div class="card-category">{{folder_name}}</div>\\n            <h3 class="card-title">{{title}}</h3>\\n            <div class="card-excerpt">{{excerpt}}</div>\\n            <div class="card-footer"><span>📅 {{date_str}}</span></div>\\n        </a>'.format(safe_url=safe_url, folder_name=folder_name, title=title, excerpt=excerpt, date_str=date_str)
    filter_buttons = ""
    for cat in sorted(categories):
        filter_buttons += f'<button class="filter-btn" data-filter="{{cat}}">{{cat}}</button>\\n        '.format(cat=cat)
    update_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    final_html = HTML_TEMPLATE.format(total_count=total_count, filter_buttons=filter_buttons, cards_html=cards_html, update_time=update_time)
    with open('index.html', 'w', encoding='utf-8') as f: f.write(final_html)
    print(f"✅ 成功生成精美主页！共处理 {{total_count}} 份报告。".format(total_count=total_count))
