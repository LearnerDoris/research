...
def run():
    ...
    with open('index.html', 'w', encoding='utf-8') as f: f.write(


cd /Users/doriswu/Documents/research
    ...
    for root, _, files in os.walk('.'):
        ...
        for file in files:
            if file.endswith('.html') and file != 'index.html':
                ...
                cats.add(cat)
                count += 1
                cards += f'<a href="{url}" class="card" data-cat="{cat}"><div class="card-cat">{cat}</div><h3 class="card-title">{title}</h3><div class="card-excerpt">{excerpt}</div><div class="card-date">📅 {date}</div></a>'
    
    f_btns = "".join([f'<button class="filter-btn" data-filter="{c}">{c}</button>' for c in sorted(cats)])
    html = HTML_TEMPLATE.replace('__TOTAL__', str(count)).replace('__FILTERS__', f_btns).replace('__CARDS__', cards).replace('__TIME__', datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    with open('index.html', 'w', encoding='utf-8') as f: f.write(html)
    print("✅ 成功修复！已去除多余符号，卡片将完美对齐。")
