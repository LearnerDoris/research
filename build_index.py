    ...
    for root, _, files in os.walk('.'):
        ...
        for file in files:
            if file.endswith('.html') and file != 'index.html':
                ...
                count += 1
                cards += f'<a href="{url}" class="card" data-cat="{cat}">\n'
                cards += f'  <div class="card-cat">{cat}</div>\n'
                cards += f'  <h3 class="card-title">{title}</h3>\n'
                cards += f'  <div class="card-excerpt">{excerpt}</div>\n'
                cards += f'  <div class="card-date">📅 {date}</div>\n'
                cards += f'</a>\n'
    
    f_btns = "".join([f'<button class="filter-btn" data-filter="{c}">{c}</button>\n        ' for c in sorted(cats)])
    
    html = HTML_TEMPLATE.replace('__TOTAL__', str(count))
    html = html.replace('__FILTERS__', f_btns)
